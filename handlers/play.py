...
async def generate_cover(requested_by, title, views, duration, thumbnail):
    ...
    image2 = Image.open("etc/RiZoeL.png")
...
async def playlist(client, message):
    ...
    msg = "**Now Playing** in {} 🤟.⚫Pᴏᴡᴇʀᴇᴅ Bʏ 𝙉𝙊𝘽𝙄 𝙭 𝙍𝙄𝙕𝙊𝙀𝙇 ".format(message.chat.title)
    msg += "\n- "+ now_playing
    msg += "\n- Req by "+by
    temp.pop(0)
    if temp:
...
async def play(_, message: Message):
                      ...
                      if administrator == message.from_user.id:  
                          ...
                          try:
                              ...
                              await lel.edit(
                                  ...
                                  "\n\nOr manually add @Michael_vcplayer to your Group and try again</b>",
                              )
                              pass
    try:
        chatdetails = await USER.get_chat(chid)
    ...
    if message.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        photo="final.png",
        reply_markup=keyboard,
        caption="▶️ **Playing** here the song requested by {} via 𝚁𝚒𝚉𝚘𝚎x𝙽𝚘𝚋𝚒 🖤".format(
...
async def deezer(client: Client, message_: Message):
    ...
    if message_.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        appendable = [s_name, r_by, loc]
        qeue.append(appendable)
        await res.edit_text(f"✧NoBixRiZoeL✧= #️⃣ Queued at position {position}")
    else:
        await res.edit_text("✧NoBixRiZoeL =▶️ Playing.....")
...
async def jiosaavn(client: Client, message_: Message):
    ...
    if message_.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        m = await client.send_photo(
            ...
            reply_markup=keyboard,
            photo="final.png",
            caption=f"✧NoBi_RiZoeL✧=#️⃣ Queued at position {position}",
        
        )           
           
    else:
        await res.edit_text("✧NoBi_RiZoeL✧=▶️ Playing.....")
        chat_id = message_.chat.id
        que[chat_id] = []
        qeue = que.get(message_.chat.id)
        s_name = sname
